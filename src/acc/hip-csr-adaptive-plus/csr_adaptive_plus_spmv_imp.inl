...
template <int REDUCE_OPTION, int WF_SIZE, int VEC_SIZE, int MAX_ROWS_PER_BLOCK, int R, int THREADS, typename I,
          typename T>
__global__ void line_enhance_plus_kernel(const I m, const I bp[], const T alpha, const T beta,
...
#define LE_PLUS_KERNEL_WRAPPER(REDUCE, MAX_ROWS_PER_BLOCK, VEC_SIZE, R, BLOCKS, THREADS)                               \
  (line_enhance_plus_kernel<REDUCE, __WRAP_SIZE__, VEC_SIZE, MAX_ROWS_PER_BLOCK, R, THREADS, int,                      \
                            double>)<<<BLOCKS, THREADS>>>(m, break_points, alpha, beta, rowptr, colindex, value, x,    \
                                                          y);
