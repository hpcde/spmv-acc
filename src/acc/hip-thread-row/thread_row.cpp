...
void thread_row_sparse_spmv(int trans, const int alpha, const int beta, int m, int n, const int *d_row_ptr,
  ...
  if (avg_nnz_per_row <= 4) {
    ...
    if (THREAD_ROW_OPT_LEVEL == THREAD_ROW_OPT_LEVEL_BLOCK) { // thread-row block level
      ...
      (kernel_thread_row_block_v3<1, MAX_ROW_NNZ, 64, THREADS_PER_BLOCK, int, double>)<<<BLOCKS, THREADS_PER_BLOCK>>>(
          alpha, beta, m, d_row_ptr, d_csr_col_index, d_csr_value, d_x, d_y);
      ...
      (kernel_thread_row<1, MAX_ROW_NNZ, 64, 256, int, double>)<<<BLOCKS, 256>>>(
          alpha, beta, m, d_row_ptr, d_csr_col_index, d_csr_value, d_x, d_y);
    }
  } else {
