...
__global__ void device_sparse_spmv_acc(int trans, const int alpha, const int beta, int m, int n, const int *rowptr,
		...
		for(int i = hipBlockIdx_x; i < m ; i += blocks){
			...
			__syncthreads();
			if(hipThreadIdx_x < 128) sum_row[hipThreadIdx_x] += sum_row[hipThreadIdx_x + 128];
		//	__syncthreads();
			if(hipThreadIdx_x < 64) sum_row[hipThreadIdx_x] += sum_row[hipThreadIdx_x + 64];
		//	__syncthreads();
			if(hipThreadIdx_x < 32) sum_row[hipThreadIdx_x] += sum_row[hipThreadIdx_x + 32];
		//	__syncthreads();
			if(hipThreadIdx_x < 16) sum_row[hipThreadIdx_x] += sum_row[hipThreadIdx_x + 16];
		//	__syncthreads();
			if(hipThreadIdx_x < 8) sum_row[hipThreadIdx_x] += sum_row[hipThreadIdx_x + 8];
		//	__syncthreads();
			if(hipThreadIdx_x < 4) sum_row[hipThreadIdx_x] += sum_row[hipThreadIdx_x + 4];
		//	__syncthreads();
			if(hipThreadIdx_x < 2) sum_row[hipThreadIdx_x] += sum_row[hipThreadIdx_x + 2];
		//	__syncthreads();
			if(hipThreadIdx_x < 1) sum_row[hipThreadIdx_x] += sum_row[hipThreadIdx_x + 1];
			__syncthreads();
			if(hipThreadIdx_x == 0) y[i] = alpha * sum_row[0] + beta * y[i];
		}								   
}
